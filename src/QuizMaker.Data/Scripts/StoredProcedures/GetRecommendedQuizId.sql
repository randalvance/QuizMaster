...
BEGIN
	...
	FROM
	(
		SELECT 1 AS TypeIndicator, Q.QuizId, Q.ModifyDate, GETDATE() AS DateCompleted, 0 AS Score FROM Quizes Q
		LEFT JOIN dbo.QuizSessions QS ON QS.QuizId = Q.QuizId
		LEFT JOIN dbo.Sessions S ON S.SessionId = QS.SessionId
		WHERE S.ApplicationUserId <> @UserId

		UNION

		SELECT 2 AS TypeIndicator, Q.QuizId, Q.ModifyDate, S.DateCompleted, S.CorrectAnswerCount AS Score FROM Quizes Q
		LEFT JOIN dbo.QuizSessions QS ON QS.QuizId = Q.QuizId
		LEFT JOIN dbo.Sessions S ON S.SessionId = QS.SessionId
